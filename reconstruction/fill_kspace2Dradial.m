    ...
    for r=1:nr_reps                   % loop through all repetitions
        ...
        for y=1:nr_ksteps            % loop through all the phase-encoding steps
            ...
            if (card_bin_ass(cnt) > 0) && (includewindow(cnt) == 1)      % if assigment = 0, this acquisition is discarded
                ...
                kspace(resp_bin_ass(cnt),card_bin_ass(cnt),slice,kline,:,dyn_bin_ass(cnt)) = kspace(resp_bin_ass(cnt),card_bin_ass(cnt),slice,kline,:,dyn_bin_ass(cnt)) + unsorted_kspace(1,r,slice,y,:,1)*exp(-1i*phaseoffset(cnt));   % add the data to the correct k-position
...
imshow(squeeze(real(kspace(1,1,1,:,:))),[]);
